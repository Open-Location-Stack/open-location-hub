...
func fromLookupEnv(lookup lookupEnvFunc) (Config, error) {
	cfg := Config{
		...
		Telemetry: TelemetryConfig{
			...
			ServiceName:           strings.TrimSpace(envWithLookup(lookup, "OTEL_SERVICE_NAME", "open-location-hub")),
		...
		Auth: AuthConfig{
			...
			Audience:            csvEnvWithLookup(lookup, "AUTH_AUDIENCE", "open-location-hub"),
