...
- current repository behavior is bounded to single-hub trackable-versus-trackable collision evaluation on the latest normalized motion state; WGS84 is preferred when available and local motion is used as a fallback for local-only streams
